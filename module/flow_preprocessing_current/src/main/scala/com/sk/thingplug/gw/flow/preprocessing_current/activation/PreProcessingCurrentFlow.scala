...
/**
  ...
  */
class PreProcessingCurrentFlow(startTime: Long) extends GraphStage[FlowShape[GraphData, GraphData]] with StrictLogging {
  ...
  //new GraphStageLogic(shape) {
    new TimerGraphStageLogic(shape) {
      ...
      override protected def onTimer(timerKey: Any): Unit = {
        ...
        logger.info(s"pushed in RuleEngineFlow($startTime ms)[${count.incrementAndGet()}] : ${key._2} ms, ${sdf.format(new Date(key._2))}")
        push(out, key._1)
      }
    }
}
